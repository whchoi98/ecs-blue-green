...
- Verify commands are copy-paste ready against actual scripts in `scripts/` and `demos/`

### 3. Architecture Doc Sync
- Update `docs/architecture.md` to reflect current CDK stacks in `lib/*.ts`
- Add new stacks, update data flows, reflect infrastructure changes
- Re-verify ASCII diagram matches actual topology

### 4. Module CLAUDE.md Audit
- Scan `lib/`, `app/`, `bin/`, `demos/`, `scripts/`, `test/`
...
- Verify runbook coverage for: CDK deploy, rollback, Blue/Green switch
- Flag stale ADRs and outdated runbooks

### 6. README.md Sync
- Update project structure section to match actual directory layout
- Sync both English and Korean sections
