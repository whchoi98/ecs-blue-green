...
LOGO
  ...
  printf '  %b%b [3] %b  %bShift Traffic (Blue↔Green)%b     %bALB weighted%b\n' "$BOLD" "$BG_G$W" "$RESET" "$W$BOLD" "$RESET" "$D" "$RESET"
  printf '  %b%b [4] %b  %bRollback to Blue%b                %b(weight 100/0)%b\n' "$BOLD" "$BG_Y$W" "$RESET" "$W$BOLD" "$RESET" "$D" "$RESET"
  printf '  %b%b [5] %b  %bWatch Traffic Monitor%b           %b(별도 터미널)%b\n' "$BOLD" "$BG_M$W" "$RESET" "$W$BOLD" "$RESET" "$D" "$RESET"
  echo
  printf '  %b [q] %b  Quit\n' "$BG_R$W$BOLD" "$RESET"
  echo
  hr '=' "$W$BOLD"
...
while true; do
  draw_menu
  printf '  %bSelect [1-5, q]: %b ' "$W$BOLD" "$RESET"
  read -r choice
  case "$choice" in
    ...
    4) run_scenario "scenario4-rollback-to-blue.sh" ;;
    5) run_scenario "watch-bluegreen-traffic.sh" ;;
    q|Q) echo; exit 0 ;;
